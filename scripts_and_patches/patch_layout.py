...
# Make TabFilamentos and TabPedidos NOT inherit from ctk.CTkFrame, but rather just be an object that creates its UI in parent.
# Wait, if they don't inherit, `self` can't be used as a master. So I need to change `self` to `self.main_container` in __init__
# Or I just use `pack` with `pack_propagate(False)` on the tab frame!

# Actually, the simplest fix is to force the tab frame to NOT expand, but then list_frame can't expand.
# Let's fix the paddings and use the grid system again but with extreme prejudice (forcing minsize/maxsize).
...
code = re.sub(r'class TabFilamentos\(ctk\.CTkFrame\):.*?(?=class TabAcervo)', '''class TabFilamentos(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.top_frame.grid(row=0, column=0, sticky="new", padx=20, pady=(5,0))
        
        self.alert_frame = ctk.CTkFrame(self.top_frame, fg_color="transparent")
        self.alert_frame.pack(side="top", fill="x")
        
        # Borda verde para provar que a versao mudou e paddings amassados
        self.form_card = ModernCard(self.top_frame, border_color="#2b7a4b", border_width=2)
        self.form_card.pack(side="top", fill="x", pady=(5, 5))
        self.form_card.grid_columnconfigure((0,1,2,3), weight=1)
        
        ctk.CTkLabel(self.form_card, text="Cadastrar Novo Filamento", font=ctk.CTkFont(size=16, weight="bold")).grid(row=0, column=0, columnspan=4, pady=(5, 5))
        
        self.marca_var = ctk.StringVar()
        f1 = ctk.CTkFrame(self.form_card, fg_color="transparent"); f1.grid(row=1, column=0, padx=10, pady=2, sticky="new")
        ...
        f2 = ctk.CTkFrame(self.form_card, fg_color="transparent"); f2.grid(row=1, column=1, padx=10, pady=2, sticky="new")
        ...
        f3 = ctk.CTkFrame(self.form_card, fg_color="transparent"); f3.grid(row=1, column=2, padx=10, pady=2, sticky="new")
        ...
        f4 = ctk.CTkFrame(self.form_card, fg_color="transparent"); f4.grid(row=1, column=3, padx=10, pady=2, sticky="new")
        ...
        f_preco = ctk.CTkFrame(self.form_card, fg_color="transparent"); f_preco.grid(row=2, column=0, padx=10, pady=2, sticky="new")
        ...
        f_link = ctk.CTkFrame(self.form_card, fg_color="transparent"); f_link.grid(row=2, column=1, padx=10, pady=2, sticky="new")
        ctk.CTkLabel(f_link, text="Link (Opcional)", text_color="gray", font=ctk.CTkFont(size=11)).pack(anchor="w")
        ctk.CTkEntry(f_link, textvariable=self.link_var, height=25).pack(fill="x")

        self.foto_path = None
        f5 = ctk.CTkFrame(self.form_card, fg_color="transparent"); f5.grid(row=2, column=2, padx=10, pady=2, sticky="new")
        ...
        ctk.CTkButton(self.form_card, text="Salvar", height=25, font=ctk.CTkFont(weight="bold"), 
                      command=self.save_filamento, fg_color="#2b7a4b").grid(row=2, column=3, padx=10, pady=(15, 2), sticky="new")
        
        ctk.CTkFrame(self.form_card, height=5, fg_color="transparent").grid(row=3, column=0)
    ...
    def edit_item(self, id, r_marca, r_mat, r_cor, r_preco):
        ...
        ctk.CTkButton(dialog, text="Salvar Alterações", fg_color="#2b7a4b", command=save).pack(pady=20)

    def delete_item(self, id):
        if messagebox.askyesno("Confirmar", "Tem certeza que deseja remover este filamento?"):
            conn = sqlite3.connect("print_manager_v2.db")
    ...
    def load_filamentos(self):
        ...
        for row in rows:
            ...
            if row[7]: ctk.CTkButton(top, text="Link", width=30, height=20, fg_color="transparent", text_color="#2b7a4b", command=lambda url=row[7]: open_url(url)).pack(side="left")
            ...
            pb = ctk.CTkProgressBar(card, width=120, height=5, progress_color="#2b7a4b" if prog > 0.2 else "#d64545")
            pb.pack(pady=5)
            pb.set(prog)
...
code = re.sub(r'class TabPedidos\(ctk\.CTkFrame\):.*?(?=class TabFinanceiro)', '''class TabPedidos(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.form_card = ModernCard(self.top_frame, border_color="#2b7a4b", border_width=2)
        self.form_card.pack(fill="x", pady=(5, 5))
        ...
        ctk.CTkButton(f1, text="Salvar", height=25, fg_color="#2b7a4b", command=self.criar_pedido).pack(side="right", padx=5)
        ...
        self.kanban_frame.grid(row=1, column=0, sticky="nsew", padx=20, pady=(0, 20))
    ...
    def load_pedidos(self):
        ...
        for r in rows:
            ...
            card = ModernCard(parent, border_color="#2b7a4b" if status=="Imprimindo" else "#333")
            ...
            if valor > 0: ctk.CTkLabel(card, text=f"R$ {valor:.2f}", text_color="#2b7a4b", font=ctk.CTkFont(size=11)).pack()
