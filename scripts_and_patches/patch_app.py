...
# Modify App routing methods to NOT call .pack() if the current_frame handles its own packing, or we just remove .pack() from show_* methods.
# Actually, the simplest fix is to KEEP TabFilamentos as a frame, but set `pack_propagate(False)`? No, if it expands, it already doesn't propagate.

# Let's try placing top_frame and list_frame using the ultimate "no centering" method in CTkFrame:
# When using pack() with side="top", it should just work.
# Let's change TabFilamentos to use pure pack:
...
code = re.sub(r'class TabFilamentos\(ctk\.CTkFrame\):.*?(?=class TabAcervo)', '''class TabFilamentos(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.top_frame.pack(side="top", fill="x", anchor="n", padx=20, pady=(5,0))
        
        self.alert_frame = ctk.CTkFrame(self.top_frame, fg_color="transparent")
        self.alert_frame.pack(side="top", fill="x")
        
        self.form_card = ModernCard(self.top_frame, border_color="#d64545", border_width=2) # VERMELHO PARA PROVAR
        self.form_card.pack(side="top", fill="x", anchor="n", pady=(5, 5))
        ...
        ctk.CTkButton(self.form_card, text="Salvar", height=25, font=ctk.CTkFont(weight="bold"), 
                      command=self.save_filamento, fg_color="#d64545").grid(row=2, column=3, padx=10, pady=(15, 2), sticky="new")
    ...
    def select_photo(self):
        ...
        if path:
            self.foto_path = path
            self.btn_foto.configure(text="Anexada", fg_color="#d64545")
    ...
    def edit_item(self, id, r_marca, r_mat, r_cor, r_preco):
        ...
        ctk.CTkButton(dialog, text="Salvar", fg_color="#d64545", command=save).pack(pady=20)
...
code = re.sub(r'class TabPedidos\(ctk\.CTkFrame\):.*?(?=class TabFinanceiro)', '''class TabPedidos(ctk.CTkFrame):
    def __init__(self, parent):
        ...
        self.form_card = ModernCard(self.top_frame, border_color="#d64545", border_width=2)
        ...
        ctk.CTkButton(f1, text="Salvar", height=25, fg_color="#d64545", command=self.criar_pedido).pack(side="right", padx=5)
    ...
    def load_pedidos(self):
        ...
        for r in rows:
            ...
            card = ModernCard(parent, border_color="#d64545" if status=="Imprimindo" else "#333")
            ...
            if valor > 0: ctk.CTkLabel(card, text=f"R$ {valor:.2f}", text_color="#d64545", font=ctk.CTkFont(size=11)).pack()
